%-------------------------------------------------------------------------------
%	SECTION TITLE
%-------------------------------------------------------------------------------
\cvsection{Projects}


%-------------------------------------------------------------------------------
%	CONTENT
%-------------------------------------------------------------------------------
\begin{cventries}

%---------------------------------------------------------
  \cventry
    {Supervised by Prof.\ Michael L.\ Scott} % Status
    {Nonblocking Persistent Memory Allocator} % Project Title
    {University of Rochester} % Location
    {Aug. 2018 - Present} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Designing a nonblocking persistent memory allocator for non-volatile memory (NVM).}
      \end{cvitems}
	}

%---------------------------------------------------------
  \cventry
    {Supervised by Prof.\ Michael L.\ Scott} % Status
    {Benchmark Suite for Persistent STM} % Project Title
    {University of Rochester} % Location
    {Mar. 2018 - Present} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Developing a benchmark suite to analyze the performance of failure atomicity mechanisms for persistent memory, including concurrent data structures, transactional applications, in-memory databases, and key-value stores.}
      \end{cvitems}
    }

%---------------------------------------------------------
  \cventry
    {Supervised by Prof.\ Michael L.\ Scott} % Status
    {Persistent Word-Based STM} % Project Title
    {University of Rochester} % Location
    {Mar. 2018 - Present} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Building a persistent word-based STM that is nonblocking.}
        \item {Integrating benchmarks and other systems we are comparing against to evaluate our new STM.}
      \end{cvitems}
    }

%---------------------------------------------------------
  \cventry
    {Supervised by Prof.\ Michael L.\ Scott} % Status
    {Interval-Based Memory Reclamation} % Project Title
    {University of Rochester} % Location
    {Jan. 2017 - Aug. 2017} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Designed nonblocking reclamation schemes that avoid unbounded memory reservation with low overhead.}
        \item {Set up experiments and coauthored a paper in PPoPP'2018.}
      \end{cvitems}
    }

%---------------------------------------------------------
  \cventry
    {Supervised by Prof.\ Michael L.\ Scott} % Status
    {Synchronization Framework for Building Ordered Maps} % Project Title
    {University of Rochester} % Location
    {Jan. 2017 - Present} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Helped develop a framework for building nonblocking trees.}
        \item {Investigated and implemented various concurrent data structures.}
      \end{cvitems}
    }

%---------------------------------------------------------
  \cventry
    {Supervised by Prof.\ Jiebo Luo} % Status
    {Personalized Computerized Cognitive Training (CCT) Program} % Project Title
    {University of Rochester} % Location
    {Jan. 2017 - Apr. 2017} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Investigated the correlation between ECGs and performance in CCT.}
      \end{cvitems}
    }

%---------------------------------------------------------
  \cventry
    {Bachelor's Thesis} % Status
    {3D Rendering System} % Project Title
    {Xidian University} % Location
    {Jan. 2016 - Jun. 2016} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Developed a 3D interactive rendering system for STL files.}
        \item {Designed a topological reconstructing algorithm to correct facet normals.}
      \end{cvitems}
    }

%---------------------------------------------------------
  \cventry
    {Supervised by Dr.\ Haihong Zheng} % Status
    {Human Age Estimation from Face Images} % Project Title
    {Xidian University} % Location
    {Sep. 2014 - Dec. 2014} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Studied in human age estimation, and implemented an estimation system.}
      \end{cvitems}
    }

%---------------------------------------------------------
\end{cventries}
